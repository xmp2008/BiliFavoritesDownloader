...
#RSShub 地址 
rssURL="http://127.0.0.1:1200/bilibili/$1/$2/$3 -q -O -"
#脚本存放地址
scriptLocation="xxx/xxx/"
#视频存放地址
videoLocation="xxx/xxx/$4/"
#邮件地址
mailAddress="xxx@xx.com"
#CookieCloud的url
cookieCloudUrl="http://127.0.0.1:12088"
#CookieCloud的用户uid
uid="uid"
#CookieCloud的用户password
password="password"
#http和https代理
proxy_http="http://x.x.x.x:7890"



# cookies失效自动更新
function updateCookies() {
    #通过CookieCloud获取存储的服务器的cookies
  curl -X POST -d "password=$password" "$cookieCloudUrl/get/$uid" | jq '.cookie_data' | jq -r '.["bilibili.com"][] | "\(.name)=\(.value)"' | tr '\n' ';' | sed 's/;$//' >> "$scriptLocation"cookies.txt
  return 0
}
...
if [ "$pubdate" != "$olddate" ] && [ "$result" != "" ] && [ "$result6" = "" ]; then
    ...
    data=${substat%%#*}
    quality=${data%%Size*}
    if echo "$quality" | grep -q "error"; then
        echo "B站Cookies失效"
        #发送邮箱提醒
        echo "B站Cookies失效,请及时更新" | mailx -s "B站Cookies失效" $mailAddress
        #自动更新B站Cookies
        updateCookies
        exit 1
  fi
    if [[ $quality =~ "1080P" ]]; then
      ...
      messageMail=${messageMail}"Title: "${title}$'\n'"Quality: "${quality}$'\n'"Size: "${size}$'\n\n' #邮件方式
      message=${message}"Title:%20"${title}"%0AQuality:%20"${quality}"%0ASize:%20"${size}"%0A%0A" #telegram方式
    done
    #发送开始下载邮件（自行修改邮件地址）
        echo "$messageMail" | mailx -s "BFD：开始下载" $mailAddress
        curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/sendMessage" -d chat_id=$telegram_chat_id -d parse_mode=html -d text="<b>BFD：开始下载</b>%0A%0A$message"
            ...
            if [ $? -eq 0 ]; then
                ...
                for file in "$videoLocation$name"/*; do
                    if [ "${file##*.}" = "xml" ]; then
                        ...
                        videomessageMail=${videomessageMail}"Title: "${videoname}$'\n'"Size: "${videosize}$'\n\n'  #邮件方式
                        videomessage=${videomessage}"Title:%20"${videoname}"%0ASize:%20"${videosize}"%0A%0A" #telegram方式
          fi
        done
                #发送下载完成邮件
                echo "$videomessageMail" | mailx -s "BFD：下载完成" $mailAddress
                curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/sendMessage" -d chat_id=$telegram_chat_id -d parse_mode=html -d text="<b>BFD：下载完成</b>%0A%0A$videomessage"
                #上传至OneDrive 百度云
                ...
                if [ "$count" != "1" ]; then
                    ...
                    #发送通知
                    echo "$name" | mailx -s "BFD：下载失败" $mailAddress  #邮件
                    curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/sendMessage" -d chat_id=$telegram_chat_id -d parse_mode=html -d text="<b>BFD：下载失败</b>"
                    exit
        fi
      fi
        ...
        second="start"
        secondResult=$(curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/sendMessage" -d chat_id=$telegram_chat_id -d parse_mode=html -d text="$second")
        ...
        while true; do
            ...
            result=$(curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/editMessageText" -d chat_id=$telegram_chat_id -d message_id=$messageID -d text="$text")
        ...
        # 原脚本在此处发送Cookies失效通知
        curl --proxy $proxy_http -s -X POST "https://api.telegram.org/bot$telegram_bot_token/sendMessage" -d chat_id=$telegram_chat_id -d parse_mode=html -d text="<b>BFD：Cookies 文件失效，请更新后重试</b>%0A%0A$videomessage"
  fi
fi
